...
import { loadTsNode, willRunWithTypescript, } from "./internal/core/typescript-support.js";
import { disableReplWriterShowProxy, isNodeCalledWithoutAScript, } from "./internal/util/console.js";
if (!HardhatContext.isCreated()) {
    ...
    if (willRunWithTypescript(hardhatArguments.config)) {
        await loadTsNode(hardhatArguments.tsconfig /* , hardhatArguments.typecheck */);
    }
    const { resolvedConfig, userConfig } = await loadConfigAndTasks(hardhatArguments);
    const env = new Environment(resolvedConfig, hardhatArguments, ctx.tasksDSL.getTaskDefinitions(), ctx.tasksDSL.getScopesDefinitions(), ctx.environmentExtenders, ctx.experimentalHardhatNetworkMessageTraceHooks, userConfig, ctx.providerExtenders);
