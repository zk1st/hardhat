// Based on Reth
// https://github.com/paradigmxyz/reth/blob/6ad221fd282f32d4bf699c92f22d4e2478372b5a/crates/revm/revm-inspectors/src/tracing/utils.rs
// https://github.com/paradigmxyz/reth/blob/6ad221fd282f32d4bf699c92f22d4e2478372b5a/LICENSE-MIT

use revm::{
    interpreter::{opcode, CreateInputs, OpCode},
    primitives::{Address, CreateScheme},
};

use crate::{hex, SpecId};
...
// Backport of REVM's `CreateInputs::created_address`
pub(crate) fn created_address(inputs: &CreateInputs, nonce: u64) -> Address {
    match inputs.scheme {
        CreateScheme::Create => inputs.caller.create(nonce),
        CreateScheme::Create2 { salt } => inputs
            .caller
            .create2_from_code(salt.to_be_bytes(), &inputs.init_code),
    }
}
